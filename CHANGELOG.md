...
### Added — AI Constructability Review (Slice 1)
- **New "✦ AI Review" analysis** — an optional AI layer that reviews an imported **baseline** for construction sense and scope completeness, the way a senior planning consultant would. Detects the project type (Infrastructure / Industrial / Roads / Residential / Administrative / Hospital / Airport / Landscape / …) and reports, all as clearly-labelled **advisory opinion**: illogical relationships (with the engineering reason *why*) and the better logic to use (change / add / remove a link, multiple predecessors & successors supported); **missing activities** it would normally expect for that project type — each given a non-clashing suggested ID, a home WBS (or a suggested new one), and wired predecessor/successor; and a **WBS review** flagging missing branches.
- **Baseline Constructability Score /100** with a legend gauge — weighted **45% construction logic · 45% scope completeness · 10% structure & advisory load**, derived transparently from the finding counts (not an AI confidence number), with action bands (Ready to baseline / Minor / Significant / Major gaps).
- **Two modes** — Mode 1 uses the AI's own construction knowledge; Mode 2 validates against an optional **reference** schedule/template you attach, which outranks general knowledge.
- **Private by default & opt-in** — the whole app stays offline as before; the review runs only when you press **Run**, and sends only the schedule *skeleton* (activity names, durations, links, WBS) with costs and the client/project name stripped out first. Uses your own **Anthropic API key**, stored per-user on your machine (Settings), never in the app. `p6_evm` / `p6_audit` computations and the offline guarantee are untouched.
