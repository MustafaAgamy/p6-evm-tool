...
- **Critical-path comparison is now a timeline (from testing).** The finish-driving route is drawn on a real date axis — last update (**WAS**) over this update (**NOW**): the shared route in blue, the **new route in red from where it reroutes**, the dropped route in grey, with a plain-English conclusion and the **total finish slip** bracketed. Replaces the earlier WBS "boxes" (they read as too abstract). It reads your schedule's own WBS so it works for any construction type, and you can still regroup by any WBS level or activity code. Screen **and** PDF. Per-segment day-splits are deliberately not shown — attributing a slip to single activities needs a full P6 time-impact analysis, which this report does not do.
