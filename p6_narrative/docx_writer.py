...
def _render(doc, section, chrome=None):
    ...
    if kind == 'prose':
        ...
        # no image → nothing (§2 is only emitted when a layout image exists)
    elif kind == 'scope':
        if p.get('intro'):
...
def write_docx(doc_dict, path, chrome=None):
    ...
    sub = document.add_paragraph(
        f"Detailed Baseline Narrative · Baseline data date {meta.get('data_date', '—')}")
    sub.alignment = WD_ALIGN_PARAGRAPH.CENTER

    # A finished professional document — internal section notes are not written.
