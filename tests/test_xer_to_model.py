...
def test_project_name_from_root_wbs(tmp_path):
    # A7: project name must come from root WBS node, not proj_short_name
    p = tmp_path / "s.xer"; p.write_text(SAMPLE, encoding='cp1252')
    data = parse_xer(str(p))
    assert data.project['name'] == 'Tower 33'   # from PROJWBS proj_node_flag=Y
