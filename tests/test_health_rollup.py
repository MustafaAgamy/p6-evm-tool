...
def test_relationship_types_uses_the_dcma_fs_target():
    ...
    default status band is 98/90 (Ibrahim's per-check numbers)."""
    ...
    assert status_for('cpli', 95.0) == 'Review'     # 90 <= 95 < 98
    assert status_for('cpli', 98.0) == 'Pass'       # >= 98
...
def test_checks_status_counts_match_the_donut():
    ...
    assert h['counts'] == {'Pass': 1, 'Review': 5, 'Critical': 3, 'Not computed': 0}
