...
def _sched(spans):
    d = ScheduleData()
    d.activities = {
        str(i): {'id': f'A{i}', 'planned_start': s, 'planned_finish': f, 'planned_duration': w}
...
def test_three_way_shapes_and_before_leads_after():
    ...
    assert set(r) == {'periods', 'baseline', 'before', 'after'}
    n = len(r['periods'])
...
def test_three_way_empty_schedules():
    e = ScheduleData()
    assert three_way_scurve(e, e, e) == {'periods': [], 'baseline': [], 'before': [], 'after': []}
